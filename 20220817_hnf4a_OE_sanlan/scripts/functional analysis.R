...
ref.coding <- grch38 %>% filter(biotype == "protein_coding")
...
SigGenSet <- function(res_table_tb, p, lfc, lfc2){
  ...
  ids <- grch38[grch38$symbol %in% res_table_tb$gene, ]
...
## Dotplot 
#up.gene.list <- list(mta2VSctrl = GS_List[[2]])
...
#down.gene.list <- list(mta2VSctrl = GS_List[[3]])
...
gseaKEGG <- gseKEGG(geneList = foldchanges, # ordered named vector of fold changes (Entrez IDs are the associated names)
                    organism = "hsa", # supported organisms listed https://www.genome.jp/kegg/catalog/org_list.html
...
gseaKEGG_symbol <- setReadable(gseaKEGG, org.Hs.eg.db, keyType = "ENTREZID")
...
gseaplot2(gseaKEGG, geneSetID = c('hsa04973','hsa00071','hsa04975','hsa04974','hsa01212'))
ggsave("figures/gsea_hnf4_up.pdf", width = 8, height = 10)
